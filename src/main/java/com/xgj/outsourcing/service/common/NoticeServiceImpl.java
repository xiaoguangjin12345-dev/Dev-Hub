...
import com.xgj.outsourcing.enums.notice.NoticeStatus;
import com.xgj.outsourcing.enums.notice.NoticeType;
import com.xgj.outsourcing.enums.project.ProjectApproveType;
import com.xgj.outsourcing.enums.user.Role;
import com.xgj.outsourcing.mapper.NoticeMapper;
import com.xgj.outsourcing.pojo.dto.notice.NoticeApproveDTO;
import com.xgj.outsourcing.pojo.dto.notice.NoticeQueryDTO;
import com.xgj.outsourcing.pojo.entity.NoticeEntity;
    ...
    public void createHourWarningNotice(int pmId, int devId, String taskName, int estimatedHours, int actualHours, double percent){
        try{
            ...
            String content = String.format("""
                                       ...
                                       实际工时超出比例:   %.2f %%
                                       该任务开发人员：    %s
                                       预警时间：         %1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS
                                       """,
    ...
    public void createPMInvitationNotice(int devId, int pmId, String taskName){
        try{
            ...
            String content = String.format("""
                                       ...
                                       任务名称：  %s
                                       邀请时间：  %1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS
    ...
    public void createDevApplicationNotice(int pmId, int devId, String taskName){
        try{
            ...
            String content = String.format("""
                                    ...
                                    任务名称：  %s
                                    申请时间：  %1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS
    ...
    public void createUserAuditNotice(int targetUserId, int adminId, NoticeApproveDTO dto){
        try{
            ...
            String content = String.format("""
                                       ...
                                       您的账号审核结果如下：
                                       审核结果：  %s%s%s
                                       处理时间：  %1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS
    ...
    public void createProjectApprovalNotice(int pmId, int pmoId, String projectName, Byte type, NoticeApproveDTO dto){
        try{
            ...
            String content = String.format("""
                                       ...
                                       审核意见：  %s
                                       审核PMO：  %s
                                       审核时间：  %1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS%s
    ...
    public void createTaskReviewNotice(int devId, int pmId, String taskName, Integer revision, NoticeApproveDTO dto){
        try{
            ...
            String content = String.format("""
                                       ...
                                       评审结果：  %s
                                       评审意见：  %s
                                       评审时间：  %1$tY-%1$tm-%1$td %1$tH:%1$tM:%1$tS%s
