...
public class SDKForAIController {
    ...
    public <T> APIResponse<Boolean> setRedis(@RequestBody AIRedisDTO<T> vo) {
        // 写入Redis状态
        redisService.hashSet(vo.getRedisKey(), "data", vo.getData());
        redisService.hashSet(vo.getRedisKey(), "resultTime", LocalDateTime.now());
        redisService.hashSet(vo.getRedisKey(), "status", vo.getStatus());
